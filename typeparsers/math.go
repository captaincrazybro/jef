...
func evalMath(s lu.String, jef domain.Jef) (error, mathChunk) {
	...
	if checkRegex.MatchString(s.Tos()) {
		// Divides the expression into different chunks
		...
		for _, op := range operators {
			for i := 0; i < len(chunks); i++ {
				...
				if chunk.isOperator && util.TrimWhitespaces(chunk.strValue) == lu.String(op) {
					...
					resultChunk := evalOperator(chunks[i-1], chunk, chunks[i+1])
...
// splitIntoMathChunks divides the math expression into different chunks of operands and operators
func splitIntoMathChunks(line lu.String) (error, []mathChunk) {
	...
	for i, s := range line.Tos() {
		if s == '(' && !isQuote {
			...
			isQuote = !isQuote
		} else if util.RuneArrContains(operators, s) && parenthesisCount == 0 {
...
// evalOperator evaluates an operation between two math chunks
func evalOperator(firstChunk mathChunk, opChunk mathChunk, lastChunk mathChunk) mathChunk {
...
func parseMathChunkValue(chunk mathChunk, jef domain.Jef) (error, mathChunk) {
	...
	if data.GetType().GetName() != "int" && data.GetType().GetName() != "double" {
